...
def get_conditions(filters):
	...
	if filters.get("route"):
		conditions += " and so.route = %(route)s"

	if filters.get("item_code"):
		conditions += " and soi.item_code = %(item_code)s"

	if filters.get("description"):
		conditions += " and soi.description = %(description)s"

	if filters.get("customer"):
		conditions += " and so.customer = %(customer)s" 

	if filters.get("delivery_shift"):
		conditions += " and so.delivery_shift = %(delivery_shift)s"
...
def get_data(conditions, filters):
	data = frappe.db.sql(
		...
		SELECT
			...
			so.customer_name, so.delivery_shift,
			...
			so.company, soi.name, 
			sii.uom as supplied_uom, 
			soi.uom as ordered_uom, 
			soi.stock_uom as stock_uom, soi.conversion_factor,
	...
	for i in range(0,len(data)):
		if(data[i]["ordered_uom"]=="Crate"):
			data[i]["qty"]=data[i]["qty"]*data[i]["conversion_factor"]
			data[i]["ordered_uom"]="Nos"

	for i in range(0,len(data)):
		if(data[i]["supplied_uom"]=="Crate"):
			data[i]["delivered_qty"]=data[i]["delivered_qty"]*data[i]["conversion_factor"]
			data[i]["supplied_uom"]="Nos"
...
def get_columns(filters):
	columns = [
		...
		{"label": _("From Date"), "fieldname": "transaction_date", "fieldtype": "Date", "width": 90},
		{"label": _("To Date"), "fieldname": "delivery_date", "fieldtype": "Date", "width": 120},
		{"label": _("Shift"), "fieldname": "delivery_shift", "fieldtype": "Data", "width": 120},
