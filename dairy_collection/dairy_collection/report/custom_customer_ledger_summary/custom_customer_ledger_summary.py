...
class PartyLedgerSummaryReport:
    ...
    def get_columns(self):
        ...
        type = self.filters.get('type')
        if(type != "Bank" and type != "Cash"):
            columns += [{
                "label": _("Bank Receipts"),
                "fieldname": "bank_receipts",
                "fieldtype": "Currency",
                "options": "currency",
                "width": 120,
            },
    ...
    def get_data(self):
        ...
        for party, row in self.party_data.items():
            if (
                ...
                row.closing_balance += total_party_adjustment
                row.update({"adjustments": total_party_adjustment})
                out.append(row)
    ...
    def get_gl_entries(self):
        """
        Retrieves GL Entries based on filters and conditions
        """
        ...
        self.gl_entries = (
            ...
            .where(
                (gle.posting_date >= self.filters.from_date)
                & (gle.posting_date <= self.filters.to_date)
                & (conditions)
            )
            .order_by(gle.posting_date, gle.party)
            .run(as_dict=True)
        )

    def get_return_invoices(self):
        """
        Retrieves return invoices for the respective parties
        """
        if self.filters.party_type == "Customer":
    ...
    def get_party_adjustment_amounts(self):
        """
        Retrieves adjustment amounts for the respective parties
        """
        if not self.filters.get("party_adjustment_accounts"):
            return

        adjustment_accounts = self.filters.get("party_adjustment_accounts")
        conditions = self.prepare_conditions()
        gle = qb.DocType("GL Entry")

        party_adjustment_details = frappe.qb.from_(gle).select(
            gle.party,
            gle.account,
            gle.debit,
            gle.credit,
            ...
            & (gle.posting_date <= self.filters.to_date)
            & (conditions)
            & (gle.account.isin(adjustment_accounts))
        ).run(as_dict=True)

        self.party_adjustment_details = frappe._dict({})

        for entry in party_adjustment_details:
            if entry.party not in self.party_adjustment_details:
                self.party_adjustment_details[entry.party] = {}

            amount = entry.debit - entry.credit
            if entry.account not in self.party_adjustment_details[entry.party]:
                self.party_adjustment_details[entry.party][entry.account] = 0
            self.party_adjustment_details[entry.party][entry.account] += amount
...
def prepare_conditions(self):
    ...
    if self.filters.get("party"):
        # Handle multiple parties properly
        parties = tuple(self.filters.get("party"))
        conditions.append("party in %(party_list)s")
